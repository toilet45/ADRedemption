...
export const Laitela = {
  ...
  get continuumUnlocked() {
    return ImaginaryUpgrade(15).isBought && !Pelle.isDisabled("continuum") && !Kohler.isRunning;
  },
  get continuumActive() {
  ...
  get matterExtraPurchaseFactor() {
    ...
    let y =  ((1 + 0.5 * Math.pow(Decimal.pLog10(Currency.darkMatter.max) / 50, 0.4) *
      (1 + SingularityMilestone.continuumMult.effectOrDefault(0))) * (MendingUpgrade(18).isBought ? 1.5 : 1)) * (Ra.unlocks.improvedECRewards.canBeApplied && EternityChallenge(6).completions >= 1 && !Pelle.isDoomed ? EternityChallenge(6).vReward.effectValue : 1)+ x;
