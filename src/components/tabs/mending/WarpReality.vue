...
export default {
  ...
  methods: {
    update() {
      ...
      this.RaToR = Math.min(240, Ra.totalPetLevel)
      this.canWarp = ((this.MVRoR == 1e7) && (this.RaToR == 240) && (this.totalUpg == 16))
...
<template>
  <div class="l-warp-tab">
    ...
    >
      ...
      {{ formatInt(1e7) }} Multiversal Remains, and 240 total Ra Memory Levels to Warp Reality.
      <br>
      <br>
      {{ format(this.MVRoR) }} / {{ format(1e7) }} Multiversal Remains
      <br>
      {{ formatInt(this.RaToR) }} / {{ formatInt(240) }} Ra Memory Levels
      <br>
