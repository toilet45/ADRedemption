...
export default {
  ...
  data() {
    return {
      ...
      garbleTimer: 0,
      garbleKey: 0,
      showUnlockState: true,
    };
  },
  computed: {
    id() {
      return this.kohlerMilestone.id;
    },
    displayId() {
      return this.config.displayId ?? this.id;
    },
    config() {
      return this.kohlerMilestone.config;
    },
    styleObject() {
      return {
        "background-position": `-${(this.kohlerMilestone.column - 1) * 104}px -${(this.kohlerMilestone.row - 1) * 104}px`
      };
    },
    classObject() {
      return {
        ...
        "o-achievement--normal": !this.isObscured,
    ...
    indicatorClassObject() {
      return {
        ...
        "o-achievement__indicator--waiting": !this.isUnlocked && !this.isDisabled,
      };
    },
    rewardClassObject() {
      return {
  ...
  methods: {
    update() {
      this.isDisabled = false;//Pelle.disabledAchievements.includes(this.id) && Pelle.isDoomed;
      this.isUnlocked = this.kohlerMilestone.isUnlocked && !this.isDisabled;
